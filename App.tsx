...
const Footer = () => {
    ...
    const getField = (sectionId: string, key: string) => {
        const section = footerPage?.sections.find(s => s.id === sectionId);
        return section?.fields.find(f => f.key === key)?.value || '';
    };

    const description = getField('general', 'description') || 'Dedicados a transformar registros digitais em presença física, com a qualidade e o respeito que cada memória merece.';
    
    const instagramUrl = getField('social', 'instagram_url');
    const instagramVisible = getField('social', 'instagram_visible') === 'true';
    
    const facebookUrl = getField('social', 'facebook_url');
    const facebookVisible = getField('social', 'facebook_visible') === 'true';
    
    const twitterUrl = getField('social', 'twitter_url');
    const twitterVisible = getField('social', 'twitter_visible') === 'true';

    const socialOrder = getField('social', 'social_order') || 'instagram,facebook,twitter';
    const socialOrderArray = socialOrder.split(',').filter(Boolean);

    // Map of available icons for dynamic social networks
    const iconMap: Record<string, any> = {
        Instagram, Facebook, Twitter, Mail, Globe, MapPin, Phone, 
        Leaf, Heart, Truck, Layers, Info, Star, Shield, Gift, Camera, Zap, 
        Award, Calendar, Package, Recycle, Check, AlertCircle,
