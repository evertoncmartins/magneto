...
import { 
    ...
    ChevronsLeft, ChevronsRight, Ban, AlertOctagon, RotateCcw, Save, FileText, DollarSign, Calendar
} from 'lucide-react';
...
const AdminOrders: React.FC<AdminOrdersProps> = ({ orders, globalSearch, setGlobalSearch, refreshData, orderStatusFilter, setOrderStatusFilter }) => {
    ...
    const [userEditOrder, setUserEditOrder] = useState<Order | null>(null);
    ...
    const handleToggleConsent = (e: React.MouseEvent, order: Order, kitId: string, currentConsent: boolean) => {
        e.stopPropagation();
        ...
        updateOrderDetails(order.id, { items: updatedItems });
        refreshData();
    };
    ...
    const openUserEditModal = (e: React.MouseEvent, order: Order) => {
        e.stopPropagation();
        setUserEditOrder(order);
        setIsUserEditModalOpen(true);
        // Reset Dropdown state
        setShowUserDropdown(false);
        // Pre-fill search term
        const currentUser = allUsers.find(u => u.id === order.userId);
        if (currentUser) {
            setUserSearchTerm(currentUser.name);
        } else {
            setUserSearchTerm('');
        ...
        if (parts.length === 3) {
            setUserEditDate(`${parts[2]}-${parts[1]}-${parts[0]}`);
        } else {
            setUserEditDate('');
        }
    };
    ...
    const saveUserEdit = () => {
        if (userEditOrder) {
            const updates: Partial<Order> = {
                userId: userEditOrder.userId,
                customerName: userEditOrder.customerName
            };
            
            if (userEditDate) {
                const [y, m, d] = userEditDate.split('-');
                updates.date = `${d}/${m}/${y}`;
            }
            
            updateOrderDetails(userEditOrder.id, updates);
            refreshData();
            setIsUserEditModalOpen(false);
            setUserEditOrder(null);
        }
    };

    const handleSelectUserForEdit = (user: User) => {
        if (userEditOrder) {
            setUserEditOrder({ ...userEditOrder, userId: user.id, customerName: user.name });
    ...
    const openAddressEditModal = (e: React.MouseEvent, order: Order) => {
        e.stopPropagation();
    ...
    const saveAddressEdit = () => {
        if (addressEditOrder) {
            ...
            updateOrderDetails(addressEditOrder.id, updates);
            refreshData();
    ...
    return (
        <>
                            ...
                            return (
                                <div key={order.id} className={`bg-white rounded-xl border transition-all duration-300 relative overflow-hidden ${isExpanded ? 'shadow-xl border-[#B8860B]/30' : 'border-gray-100 shadow-sm hover:shadow-md'} ${order.deleted ? 'opacity-70 grayscale' : ''}`}>
                                    ...
                                    <div className="flex">
                                        ...
                                        <div className="flex-1 flex flex-col cursor-pointer select-none" onClick={() => setExpandedOrderId(isExpanded ? null : order.id)}>
                                            <div className="p-5 flex-1">
                                                <div className="flex items-start justify-between gap-4">
                                                    <div className="flex items-center gap-4">
                                                        <div className="w-12 h-12 rounded-xl bg-[#F5F5F7] flex items-center justify-center font-bold text-lg text-[#1d1d1f] shrink-0">
                                                            {order.customerName.charAt(0)}
                                                        </div>
                                                        <div className="min-w-0">
                                                            <h3 className="font-bold text-[#1d1d1f] text-sm md:text-base truncate">{order.customerName}</h3>
                                                            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-0.5">
                                                                #{order.id} • {order.itemsCount} Itens
                                                            </p>
                                                            <div className="flex gap-2 mt-1.5">
                                                                {order.userId?.includes('admin') && <span className="bg-gray-100 text-gray-500 text-[9px] px-1.5 py-0.5 rounded font-bold uppercase shrink-0">Admin</span>}
                                                                {order.createdByAdmin && <span className="bg-indigo-50 text-indigo-600 text-[9px] px-1.5 py-0.5 rounded font-bold uppercase shrink-0 flex items-center gap-1"><Shield size={8}/> Admin</span>}
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <span className={`px-2 py-1 rounded-md text-[9px] font-bold uppercase tracking-widest border whitespace-nowrap ${statusConfig.color}`}>
                                                        {statusConfig.label}
                                                    </span>
                                                </div>

                                                <div className="h-px bg-gray-50 my-4 w-full"></div>
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    {order.shippingAddress && (
                                                        <div className="flex items-center gap-2 text-xs text-gray-500">
                                                            ...
                                                            <span className="truncate">{order.shippingAddress.city} - {order.shippingAddress.state.split(' - ').pop()}</span>
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                                            ...
                                            {/* Bottom Chevron */}
                                            <div className="h-6 flex items-center justify-center text-gray-300 hover:text-[#B8860B] hover:bg-gray-50 transition-colors mb-1">
                                                <ChevronDown size={14} className={`transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`} />
                                            </div>
                                        </div>

                                        {/* Right Sidebar Actions */}
                                        <div className="w-10 md:w-12 border-l border-gray-100 bg-gray-50/30 flex flex-col items-center justify-center py-2 gap-2 shrink-0">
                                            {/* Edit User */}
                                            <button 
                                                onClick={(e) => openUserEditModal(e, order)} 
                                                className="p-2 text-gray-400 hover:text-[#B8860B] rounded-lg transition-all" 
                                                title="Editar Cliente/Data"
                                            >
                                            ...
                                            {!order.deleted && !isCancelled && (
                                                <>
                                                    <button onClick={(e) => openCancelModal(e, order)} className="p-2 text-gray-400 hover:text-red-500 rounded-lg transition-all" title="Cancelar Pedido">
                                                        <Ban size={18} strokeWidth={1.5} />
                                                    </button>
                                                    <button onClick={(e) => handleSoftDelete(e, order)} className="p-2 text-gray-400 hover:text-red-500 rounded-lg transition-all" title="Mover para Lixeira">
                                            ...
                                            {isCancelled && !order.deleted && (
                                                <>
                                                    <button onClick={(e) => openRevertModal(e, order)} className="p-2 text-gray-400 hover:text-emerald-600 rounded-lg transition-all" title="Reverter Cancelamento">
                                            ...
                                            {order.deleted && (
                                                <button onClick={(e) => handleRestore(e, order)} className="p-2 text-gray-400 hover:text-emerald-500 rounded-lg transition-all" title="Restaurar Pedido">
                                            ...
                                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                                                <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                                                    <div className="flex justify-between items-center mb-4">
                                                        <h4 className="text-[10px] font-bold text-[#B8860B] uppercase tracking-widest flex items-center gap-2"><MapPin size={14} /> Entrega</h4>
                                                        <button onClick={(e) => openAddressEditModal(e, order)} className="p-1.5 text-gray-400 hover:text-[#B8860B] hover:bg-gray-50 rounded-md transition-colors" title="Editar Endereço">
                                                            <Edit3 size={14} />
                                                        </button>
                                                    </div>
                                                    {order.shippingAddress ? (
                                                        ...
                                                        return (
                                                            <div key={kitId} className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm relative overflow-hidden">
                                                                <div className="flex items-center justify-between mb-4 flex-wrap gap-4">
                                                                    ...
                                                                    <div className="flex items-center gap-3">
                                                                        <button 
                                                                            onClick={(e) => handleToggleConsent(e, order, kitId, !!consent)}
                                                                            className={`inline-flex items-center justify-center w-8 h-8 rounded-lg border shadow-sm transition-all ${consent ? 'bg-emerald-50 text-emerald-700 border-emerald-100 hover:bg-emerald-100' : 'bg-gray-100 text-gray-500 border-gray-200 hover:bg-gray-200'}`}
                                                                            title={consent ? "Uso Autorizado (Clique para alterar)" : "Privado (Clique para alterar)"}
                                                                        >
                                                                            {consent ? <Camera size={16} /> : <Shield size={16} />}
                                                                        </button>
            ...
            {isUserEditModalOpen && userEditOrder && createPortal(
                <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-[#1d1d1f]/60 backdrop-blur-md" onClick={() => setIsUserEditModalOpen(false)}></div>
                    <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl relative flex flex-col animate-fade-in border border-gray-100">
                        <div className="px-8 py-6 border-b border-gray-100 bg-white sticky top-0 z-20 flex justify-between items-center rounded-t-2xl">
                            <div>
                                <h3 className="font-serif font-bold text-2xl text-[#1d1d1f]">Alterar Cliente</h3>
                                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">Pedido #{userEditOrder.id}</p>
                            </div>
                            ...
                            <div className="bg-white p-3 md:p-6 rounded-xl border border-gray-100 shadow-sm">
                                    ...
                                    {showUserDropdown && userSearchTerm && (
                                        <div className="absolute z-20 top-full left-0 right-0 mt-2 bg-white rounded-lg shadow-xl border border-gray-100 max-h-48 overflow-y-auto">
                                            {filteredUsersForModal.map(u => (
                                                <div key={u.id} onClick={() => handleSelectUserForEdit(u)} className="px-4 py-3 cursor-pointer hover:bg-gray-50">
                                ...
                                <div className="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-100">
                                    <p className="text-xs text-blue-800">
                                        <strong>Cliente Selecionado:</strong> {userEditOrder.customerName}
                                    </p>
                                </div>
                        ...
                        <div className="p-6 border-t border-gray-50 bg-white flex gap-4 sticky bottom-0 z-20 rounded-b-2xl">
                            <button onClick={() => setIsUserEditModalOpen(false)} className="flex-1 py-3 bg-gray-100 text-[#1d1d1f] font-bold text-[10px] uppercase tracking-widest rounded-lg hover:bg-gray-200 transition-all">Cancelar</button>
                            <button onClick={saveUserEdit} className="flex-1 py-3 bg-[#1d1d1f] text-white font-bold text-[10px] uppercase tracking-widest rounded-lg hover:bg-black transition-all flex items-center justify-center gap-2"><Save size={16}/> Salvar Alteração</button>
                        </div>
                    </div>
                </div>,
                document.body
                             ...
                             {activeTab === 'summary' && (
                                <div className="space-y-6">
                                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                                        ...
                                        <div className="relative">
                                            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={16}/>
                                            <input type="text" placeholder="Buscar..." className={`${inputClasses} pl-12`} value={userSearchTerm} onChange={(e) => { setUserSearchTerm(e.target.value); setShowUserDropdown(true); }} onFocus={() => setShowUserDropdown(true)} ref={userSearchInputRef} />
                                            {/* Dropdown Logic */}
                                            {showUserDropdown && userSearchTerm && (
                                                <div className="absolute z-20 top-full left-0 right-0 mt-2 bg-white rounded-lg shadow-xl border border-gray-100 max-h-48 overflow-y-auto">
                                                    {filteredUsersForModal.map(u => (
            ...
            {isAddressEditModalOpen && addressEditOrder && createPortal(
                <div className="fixed inset-0 z-[9999] bg-white animate-fade-in flex flex-col">
                    ...
                    <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-white sticky top-0 z-20">
                        <div>
                            <h2 className="font-serif font-bold text-2xl text-[#1d1d1f]">Editar Endereço de Entrega</h2>
                            <p className="text-xs text-gray-500 mt-1">Pedido #{addressEditOrder.id} • {addressEditOrder.customerName}</p>
                        </div>
                        <button 
                            onClick={() => setIsAddressEditModalOpen(false)}
                            className="p-2 text-gray-400 hover:text-[#1d1d1f] hover:bg-gray-50 rounded-full transition-colors"
                        >
                            <X size={24} />
                        </button>
                    </div>
                    ...
                    {/* Body */}
                    <div className="flex-1 overflow-y-auto p-6 md:p-10 bg-[#F9F9FA]">
                        <div className="max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {/* CEP */}
                                <div className="md:col-span-2">
                                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">CEP</label>
                                    <div className="relative">
                                        <input 
                                            type="text" 
                                ...
                                {/* Street */}
                                <div className="md:col-span-2">
                                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Rua / Avenida</label>
                                    <input 
                                        type="text" 
                                ...
                                {/* Number */}
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Número</label>
                                    <input 
                                        type="text" 
                                ...
                                {/* Complement */}
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Complemento</label>
                                    <input 
                                        type="text" 
                                        value={editAddressForm.complement || ''} 
                                        onChange={(e) => setEditAddressForm({...editAddressForm, complement: e.target.value})}
                                        className={inputClasses}
                                    />
                                </div>
                                ...
                                {/* Neighborhood */}
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Bairro</label>
                                    <input 
                                        type="text" 
                                ...
                                {/* City */}
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Cidade</label>
                                    <input 
                                        type="text" 
                                ...
                                {/* State */}
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Estado (UF)</label>
                                    <input 
                                        type="text" 
                    ...
                    <div className="p-6 border-t border-gray-100 bg-white flex justify-end gap-4 sticky bottom-0 z-20">
                        <button 
                            onClick={() => setIsAddressEditModalOpen(false)} 
                            className="px-8 py-3 bg-gray-100 text-[#1d1d1f] font-bold text-[10px] uppercase tracking-widest rounded-lg hover:bg-gray-200 transition-all"
                        >
                            Cancelar
                        </button>
                        <button 
                            onClick={saveAddressEdit} 
                            className="px-8 py-3 bg-[#1d1d1f] text-white font-bold text-[10px] uppercase tracking-widest rounded-lg hover:bg-black transition-all flex items-center gap-2 shadow-lg"
                        >
