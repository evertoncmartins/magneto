...
import { X, Package, Calendar, Clock, CheckCircle, Truck, Box } from 'lucide-react';
                                    ...
                                    >
                                        <div className="flex justify-between items-center">
                                            <div className="flex items-center gap-4">
                                                <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${status.bg} ${status.text} shadow-inner`}>
                                                    <status.icon size={20} strokeWidth={2.5} />
                                                </div>
                                                <div>
                                                    <span className="block text-sm font-bold text-[#1d1d1f] group-hover:text-[#B8860B] transition-colors">Pedido #{order.id}</span>
                                                    <span className="block text-[10px] text-gray-400 font-bold uppercase tracking-widest flex items-center gap-1.5 mt-1">
                                                        <Calendar size={12} className="text-gray-300"/> {order.date}
                                                    </span>
                                                </div>
                                            </div>
                                            <span className={`px-3 py-1.5 rounded-lg text-[9px] font-bold uppercase tracking-[0.15em] ${status.bg} ${status.text} border border-transparent group-hover:border-current transition-all`}>
                                                {status.label}
                                            </span>
                                        </div>
