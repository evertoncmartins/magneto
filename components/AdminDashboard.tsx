              ...
              <nav className="flex-1 space-y-1 overflow-y-auto scrollbar-hide overflow-x-hidden">
                  ...
                  {isCmsSubmenuOpen && !isSidebarCollapsed && (
                      <div className="ml-4 pl-4 border-l border-gray-100 space-y-1 mt-1">
                          ...
                          {/* 2. CMS: Rodapé (Direct Link) */}
                          <button 
                              onClick={() => { setActiveTab('cms-footer'); setIsSidebarOpen(false); }}
                              className={`w-full flex items-center justify-between px-4 py-2.5 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all ${activeTab === 'cms-footer' ? 'text-[#B8860B] bg-[#FFF9E6]' : 'text-gray-400 hover:text-[#1d1d1f] hover:bg-gray-50'}`}
                          >
                              <div className="flex items-center gap-3">
                                  <LayoutGrid size={14} /> Rodapé
                              </div>
                          </button>
